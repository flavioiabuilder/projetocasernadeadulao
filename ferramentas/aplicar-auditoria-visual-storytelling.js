...
function once(needle, repl, label) {
...
once('--barra:46px;', '--barra:44px;', 'barra-44');
...
// S19 remove fecho
const s19start = h.indexOf('id="s19"');
const s20start = h.indexOf('id="s20"');
if (s19start !== -1 && s20start !== -1) {
  let chunk = h.slice(s19start, s20start);
  if (chunk.includes('class="fecho"')) {
    chunk = chunk.replace(/<p class="fecho">[\s\S]*?<\/p>/, '');
    h = h.slice(0, s19start) + chunk + h.slice(s20start);
    console.log('OK: s19 fecho removed');
  } else console.log('SKIP: s19 no fecho');
}

// S01 seta
const s01a = h.indexOf('id="s01"');
const s02a = h.indexOf('id="s02"');
if (s01a !== -1 && s02a !== -1) {
  const chunk = h.slice(s01a, s02a);
  if (!chunk.includes('seta-rolar')) {
    ...
    // insert before closing bloco-central
    ...
    if (h.includes(needle)) {
      ...
      // try generic after emblema in s01
      const imgEnd = h.indexOf('>', h.indexOf('emblema-grande', s01a));
      // find first </div> after img within s01 - fragile
...
const checks = {
  barra44: v.includes('--barra:44px'),
  chrome: v.includes('chrome-escuro'),
  space: v.includes('emControle'),
  fechoStatic: v.includes('.fecho{position:static'),
  d1: v.includes('76px'),
  stagger: v.includes('animation-delay:80ms'),
