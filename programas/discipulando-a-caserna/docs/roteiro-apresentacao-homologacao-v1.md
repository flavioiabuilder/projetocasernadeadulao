# Roteiro da Apresentação Institucional

## Discipulando a Caserna — Projeto Caserna de Adulão

**Destinatário primário:** Pr. Glaydston Gama Lopes (pastor-presidente / validador pastoral)
**Natureza da peça:** instrumento de homologação pastoral da v1.0-RC do Guia Mestre
**Formato:** arquivo HTML único, autocontido, navegável por scroll-snap
**Autor:** Obr. Flávio Alves da Costa
**Versão do roteiro:** v1.1 — Fortaleza, 2026

### Decisões de governança deste roteiro (v1.1)

1. **Prefácio:** não é texto do Pastor nem fato consumado. É **explicação + convite** — página reservada, sem prazo. Storytelling preservado; autoria não antecipada.
2. **Caderneta de Campanha:** proposta de instrumento para a Ordem do Dia (3.4), não exigência nomeada no Guia.
3. **Merch (Tela 24):** **exibir** nesta versão, como **estudo proposto** (selo visível); salvaguardas na mesma tela, peso tipográfico equivalente.
4. **Citações bíblicas:** NAA, forma completa quando em tela (Fp 1.6).
5. **Fonte da verdade em prosa:** Guia Mestre v1.0-RC + `conteudo/` do prospecto; campos `null` omitidos; nada inventado como homologado.

> Nota: o arquivo em `fontes/guia-mestre/` pode conter rascunho de Prefácio. **Esta peça não o apresenta como escrito nem como validação concluída.** Homologação permanece pendente (nota final do Guia).

---

## Como ler este roteiro

Cada seção abaixo corresponde a **uma tela** da apresentação. Para cada uma há quatro campos:

- **Tela** — o texto que efetivamente aparece na projeção. Está redigido para uso direto, não é resumo.
- **Visual** — o elemento gráfico ou interativo daquela tela.
- **Condução** — o que você diz ao vivo, e como a tela se comporta. Não aparece na projeção.
- **Fonte** — a seção do Guia Mestre que sustenta o conteúdo, para rastreabilidade.

Regra transversal: **nenhuma tela depende de narrador para fazer sentido.** O Pastor vai reabrir este arquivo sozinho depois. O texto em tela precisa se sustentar.

**Contagem:** 30 telas, distribuídas em 9 atos. Tempo estimado de apresentação ao vivo: 35 a 45 minutos.

**Espinha retórica:** a frase **"De quebrantados, valentes."** (Dedicatória) retorna nas Telas 06, 21 e 30.

---

# ATO 0 — A CAVERNA

> Função: silêncio antes da tese. Uma tela só. Sem menu, sem barra de progresso, sem logo.

---

### Tela 01 — Abertura

**Tela**

> "todo homem em aperto, endividado e amargurado de espírito"
>
> 1Sm 22.2

_(abaixo, muito menor, após três segundos)_

**Discipulando a Caserna**
Projeto Caserna de Adulão · Guia Mestre v1.0-RC
Apresentação institucional para validação pastoral

**Visual**
Fundo off-white #F4F4F1 quase vazio. Texto centralizado, serifa grande. A citação aparece primeiro; o bloco de identificação faz _fade-in_ depois. Nenhum ornamento. Uma seta discreta de scroll no rodapé.

**Condução**
Não fale nesta tela. Deixe o silêncio de três a cinco segundos. É a única vez na apresentação em que o silêncio é parte do argumento. Só depois: _"Pastor, é daqui que tudo começa — e o senhor sabe disso melhor do que eu."_

**Fonte** — Dedicatória; 1.2

---

# ATO I — A DOR

> Função: ancorar o porquê na realidade concreta, não na teoria. Três telas.

---

### Tela 02 — O homem que chega

**Tela**

**Ele chega com a farda e com a vergonha.**

Conhece hierarquia. Conhece disciplina. Conhece missão.
E carrega, ao mesmo tempo, a culpa real e o peso do que ainda não conseguiu nomear.

Muitos chegam acreditando que Cristo foi embora na hora da desonra.
Outros, que Ele nunca esteve ali para começo de conversa.

**Visual**
Tela dividida verticalmente. À esquerda, quatro palavras empilhadas em sans condensada, caixa alta, cor chumbo: HIERARQUIA / DISCIPLINA / MISSÃO / LEALDADE. À direita, quatro palavras na mesma posição, em serifa, cor mais fraca: CULPA / VERGONHA / RUPTURA / SILÊNCIO. Nenhuma imagem de pessoa.

**Condução**
Este é o primeiro momento de conteúdo. Diga em voz baixa. Não dramatize — o texto já carrega. Se o Pastor comentar aqui, deixe-o comentar; esta tela existe para provocar reconhecimento, não para ser vencida rápido.

**Fonte** — Nota do Autor; 1.4.1 _(linguagem do Prefácio-rascunho do Guia não é citada como autoria pastoral)_

---

### Tela 03 — O terreno

**Tela**

**Presídio Militar da PMCE.**

Horário controlado. Circulação controlada. Material controlado.
Tensão institucional. Número de participantes que varia a cada semana.

Não é um lugar onde se improvisa discipulado.
É um lugar onde o improviso custa caro.

**Visual**
Diagrama sóbrio em SVG: uma planta esquemática simplificada, sem identificar instalação real, com quatro restrições rotuladas em torno (tempo, circulação, material, rotatividade). Traço fino, sem preenchimento, cor chumbo.

**Condução**
Aqui você estabelece a credencial do documento: ele não foi escrito de gabinete. _"Tudo o que vem a seguir foi desenhado para funcionar neste terreno — não em uma sala de aula ideal."_

**Fonte** — 1.4.3; 6.7.2

---

### Tela 04 — O que esse homem não precisa

**Tela**

**Ele não precisa de mais cobrança.**

Precisa de alguém que vá à caverna com ele e aponte Cristo.

**Visual**
Tela quase vazia. Uma frase grande. Nada mais. Fundo escurece levemente em relação às telas anteriores — a única variação tonal do Ato I.

**Condução**
Pausa. Esta é a tela-pivô do primeiro ato: ela fecha o diagnóstico e abre a resposta. Frase única, dita uma vez, sem repetir.

**Fonte** — Nota do Autor

---

# ATO II — O QUE JÁ ACONTECIA

> Função: humildade institucional. O projeto não inventou o mover de Deus; organizou-se em torno dele. Duas telas.

---

### Tela 05 — Antes da estrutura

**Tela**

**Antes que houvesse estrutura, método ou currículo, o Senhor já chamava.**

Não havia matriz curricular.
Não havia módulos, símbolos, transições ou certificados.
Havia homens se assentando, Bíblia aberta, e fé reacendendo onde a culpa tentava apagar.

O Guia não criou esse mover. O Guia nasceu para **guardar** o que já estava acontecendo.

**Visual**
Linha do tempo horizontal minimalista, sem datas: um ponto marcado "o mover" bem à esquerda, e o ponto "o Guia" bem depois. O espaço entre eles é o argumento. Anima da esquerda para a direita ao entrar na tela.

**Condução**
Esta tela protege o projeto de soar tecnocrático. Diga: _"Nada aqui é invenção metodológica. É organização pastoral de algo que Deus começou antes."_ Se o Pastor tem alguma reserva quanto a "sistematizar demais", ela se dissolve aqui.

**Fonte** — Agradecimentos; 1.2

---

### Tela 06 — Adulão

**Tela**

**A caverna que dá nome a tudo.**

Davi acolheu em Adulão homens abatidos, endividados e amargurados.
Sob liderança forjada na crise, aqueles proscritos se tornaram valentes.

O projeto reconhece a mesma lógica: **a caserna também pode ser uma caverna** — e Cristo, o Comandante maior que Davi, faz de quebrantados, valentes.

**Visual**
Composição tipográfica. A palavra ADULÃO em corpo muito grande, com as três condições (aperto · dívida · amargura) dispostas como legendas ao redor, ligadas por filetes finos. Cor latão discreto apenas nas três condições.

**Condução**
Primeira aparição da frase-âncora: **"de quebrantados, valentes."** Ela volta mais duas vezes na apresentação (Telas 21 e 30). Diga-a devagar e não a explique.

**Fonte** — Dedicatória; 1.2; 1.3

---

# ATO III — O PROBLEMA

> Função: justificar a existência do documento. Sem este ato, o Guia parece burocracia. Duas telas.

---

### Tela 07 — O risco

**Tela**

**Sem método, o que Deus faz se perde.**

Turmas com estágios diferentes e nenhum critério para reconhecê-los.
Temas repetidos sem aprofundamento, e lacunas doutrinárias que ninguém percebe.
Instrutores bem-intencionados produzindo materiais incompatíveis.
Um homem que avança no presídio e, na igreja, é convidado a recomeçar do zero.

**Visual**
Quatro cartões em grade 2×2, cada um com o risco escrito. Ao passar o cursor (ou tocar), o cartão vira e mostra a salvaguarda correspondente prevista no Guia — matriz única, progressão em espiral, padrão didático fixo, remissão de módulo concluído.

**Condução**
Esta é a tela mais argumentativa da apresentação. Deixe o Pastor virar os cartões ele mesmo se estiver com o dispositivo em mãos. _"Cada risco à esquerda tem uma resposta desenhada no Guia. Nenhum deles é hipotético."_

**Fonte** — 1.2; 3.1; 3.5; 1.4.3

---

### Tela 08 — A resposta

**Tela**

**Um Guia Mestre.**

Referência oficial. Vade Mecum espiritual.
Não para engessar o que Deus faz — para guardar, nutrir e conduzir.

Quatro exigências que o documento assumiu desde a primeira linha:

**Unificar** a doutrina · **Servir** de referência ao instrutor · **Estabelecer** jornada progressiva · **Garantir** continuidade e replicabilidade

**Visual**
Mockup da capa do Guia Mestre, em perspectiva leve, com os quatro verbos dispostos ao lado. Se houver arte de capa pronta, usar; se não, composição tipográfica com o escudo.

**Condução**
Transição para o ato conceitual. _"O que vem agora é como esse documento foi pensado — não o que ele contém, mas por que ele tem a forma que tem."_

**Fonte** — 1.1; 1.2

---

# ATO IV — A CONCEPÇÃO

> Função: o momento intelectual da apresentação. É aqui que o Pastor vê a coerência doutrinária inteira. Quatro telas.

---

### Tela 09 — O eixo

**Tela**

**Não são quatro temas. São quatro ações de Cristo.**

**Cristo Chamando** — convoca, perdoa, incorpora
**Cristo Treinando** — ensina, corrige, firma fundamentos
**Cristo Moldando** — amadurece afetos, vontade e caráter
**Cristo Enviando** — comissiona, sustenta, multiplica

A ordem não é organizacional. É o próprio caminho da graça na vida do homem — e por isso não pode ser invertida.

**Visual**
Quatro blocos horizontais que se acendem em sequência ao entrar na tela. Cada um com o verbo em destaque e a referência bíblica em corpo pequeno. Elemento central da apresentação: **é aqui que a barra de armadura aparece pela primeira vez no topo**, ainda apagada.

**Condução**
Marque explicitamente: _"Repare que o sujeito de todas as quatro frases é Cristo, não o instrutor e não o discípulo. Isso não é retórica — é o que impede o programa de virar autoajuda com linguagem militar."_

**Fonte** — 1.5; 3.2

---

### Tela 10 — A Armadura

**Tela**

**Uma narrativa unifica os quatro módulos: Efésios 6.**

O discípulo não coleciona conteúdos. Ele é **revestido**, peça por peça.

Cinto da Verdade → Couraça da Justiça → Calçados do Evangelho da Paz → Escudo da Fé

**Visual**
Os quatro ícones de insígnia em linha, ainda em contorno vazado. Conforme o scroll avança dentro da própria tela, cada um se preenche em latão. Simultaneamente, a barra de armadura no topo acende a primeira peça — **estabelecendo, para o resto da apresentação, que a barra de progresso é a armadura.**

**Condução**
Aponte a barra do topo explicitamente uma única vez, aqui: _"O senhor vai reparar que a barra lá em cima vai se completando conforme avançamos. Ela é a armadura. A navegação desta apresentação é a própria doutrina do projeto."_ Não volte a mencionar depois.

**Fonte** — 1.6; 3.1

---

### Tela 11 — O que ficou de fora, e por quê

**Tela**

**Duas peças não foram atribuídas a módulos. A decisão é teológica.**

**A Espada do Espírito** — a Palavra não pertence a uma etapa. Ela atravessa todas, do chamado ao envio, como instrumento de cada lição.

**O Capacete da Salvação** — guarda a mente na esperança. Está reservado à dimensão pastoral de socorro, nos módulos complementares.

Quatro peças descrevem a **marcha**. Duas acompanham o soldado em **toda** a jornada e em suas crises.

**Visual**
Diagrama de duas camadas: embaixo, os quatro símbolos em sequência linear; acima, atravessando todos horizontalmente, a espada; e cobrindo o conjunto, o capacete. Fica visualmente evidente que são planos diferentes.

**Condução**
Tela de credibilidade. Mostra que o sistema simbólico foi pensado, não montado por conveniência estética. É provável que seja aqui que o Pastor faça a primeira pergunta técnica — deixe espaço.

**Fonte** — 1.6 (parágrafo final)

---

### Tela 12 — O método

**Tela**

**Contemplar antes de obedecer.**

Nenhuma disciplina espiritual verdadeira nasce de cobrança — nasce de fascínio.
Nenhuma obediência frutífera nasce de medo — nasce de amor.

Por isso toda lição segue a mesma ordem interna:
**ver Cristo** → **entender pela Palavra** → **responder em obediência concreta**

A prática não paga culpa. Ela responde à graça.

**Visual**
Três círculos concêntricos ou três etapas encadeadas, com a seta de retorno indicando que o ciclo se repete a cada encontro. Sóbrio, sem cores fortes.

**Condução**
Coração pastoral do método — sem atribuir Prefácio. Diga: _"Este é o critério com que cada uma das 48 lições foi revista. A página do Prefácio do Guia está em branco de propósito: quando o senhor entender que é o tempo, essa palavra será a sua — não a minha."_ Planta o convite sem quebrar o arco; o formalismo do pedido vem na Tela 29.

**Fonte** — 1.1; 1.2; 3.6; Nota do Autor _(critério metodológico do Guia; Prefácio reservado)_

---

# ATO V — A ARQUITETURA

> Função: prova de rigor. É o ato mais denso; controle o ritmo. Cinco telas.

---

### Tela 13 — A conta

**Tela**

# 4 × 12 = 48

**Quatro módulos sequenciais. Doze lições cada. Um ciclo anual.**

Nenhum módulo repete o outro. Alguns temas retornam — com profundidade e finalidade diferentes.
O discípulo não gira nos mesmos assuntos: ele sobe em espiral.

**Visual**
Número grande em tela cheia, com os quatro blocos de 12 lições representados como uma grade de 48 marcadores. Ao entrar, os marcadores se acendem em sequência rápida por módulo, com as quatro cores/tons de cada movimento.

**Condução**
Tela de impacto numérico. Não explique muito. A grade de 48 já foi vista — ela reaparece filtrável na Tela 16.

**Fonte** — 3.1; 3.2

---

### Tela 14 — O ritmo

**Tela**

**Um encontro por semana. De 1h30 a 2h.**

**48 semanas** efetivas de aula · **11 a 12 meses** de calendário
A folga existe para feriados, restrições institucionais, revisões e cerimônias de transição.

Dentro de cada encontro:
Contemplação na Palavra (15–25 min) · Discernimento e aplicação (25–35 min) · Oração e compromisso (10–15 min) · **Ordem do Dia**

**Visual**
Barra horizontal proporcional representando os blocos de tempo do encontro, com a Ordem do Dia destacada como extensão que sai da barra e se prolonga para fora — sinalizando que ela ocorre _fora_ da sala, durante a semana.

**Condução**
Enfatize o gesto visual da Ordem do Dia saindo da barra: _"O encontro termina, mas a lição não. A Ordem do Dia é o que atravessa a semana."_ Prepara a Tela 23 (Caderneta).

**Fonte** — 3.3

---

### Tela 15 — Anatomia de uma lição

**Tela**

**Toda lição tem sete elementos. Sempre os mesmos.**

Título · Texto-base · Objetivo · Síntese teológica · Aplicação militar · **Ordem do Dia** · Perguntas de reflexão

A repetição do formato não cria frieza. **Cria chão.**
Instrutores diferentes, em ambientes diferentes, ministram o mesmo conteúdo sem perda de ênfase.

**Visual**
Diagrama explodido e anotado de uma lição real — sugestão: **Lição 6, "Ordem de reintegração: a graça que devolve a patente"** (Ef 2.8–9), porque é a lição que melhor sintetiza a tese do projeto. Cada um dos sete elementos é clicável e expande com o conteúdo real daquela lição.

**Condução**
Deixe o Pastor abrir os elementos. Se o tempo estiver curto, abra apenas "Aplicação militar" e "Ordem do Dia" — são os dois que diferenciam este material de qualquer apostila genérica.

**Fonte** — 3.4; 3.9 (lição 6)

---

### Tela 16 — A Matriz Curricular

**Tela**

**As 48 lições, em visão anual.**

_Filtre por módulo para ver a progressão completa._

**Visual**
**A interação mais importante da apresentação.** Tabela filtrável: número, título, texto-base, objetivo sintético. Quatro botões de filtro (um por módulo) mais "todas". Ao filtrar, a linha do símbolo correspondente se destaca no topo. Rolagem interna, altura fixa, sem quebrar o scroll-snap da página. Dados: `conteudo/matriz-curricular.json`.

**Condução**
Esta é a tela em que você **para de apresentar e deixa o Pastor navegar.** Diga apenas: _"Fique à vontade para percorrer. Se quiser abrir alguma lição específica, o Guia completo está no anexo."_ Silêncio produtivo. Não preencha com fala.

**Fonte** — 3.9 (Quadro 1)

---

### Tela 17 — O que se espera do homem ao fim

**Tela**

**Cinco marcas — sinais de uma obra de graça, não medalhas.**

Fé bíblica e cristocêntrica · Identidade restaurada e honra redimida · Disciplina espiritual e vida em comunhão · Caráter obediente e servidor · Compromisso com a missão e a multiplicação

Nenhuma delas descreve perfeição alcançada. Todas descrevem **caminho em andamento**.

**Visual**
Cinco marcas dispostas em arco, com a ressalva final em corpo menor e destacada em faixa. A ressalva é tão importante quanto as marcas — não a diminua visualmente demais.

**Condução**
Frisar a última linha em voz alta. É a salvaguarda pastoral que impede o perfil de virar régua de cobrança. _"Isso não é um checklist de aprovação. É um mapa de leitura pastoral."_

**Fonte** — 3.8

---

# ATO VI — A MARCHA

> Função: o coração simbólico. Depois da densidade do Ato V, o tom volta a ser afetivo. Quatro telas.

---

### Tela 18 — As quatro marchas

**Tela**

**Cada módulo termina em um marco reconhecido pela tropa.**

**Primeira Marcha** — O Recruta que se Rendeu
**Segunda Marcha** — O Combatente que se Fortalece
**Terceira Marcha** — O Guerreiro que Persevera
**Formatura do Soldado de Cristo**

**Visual**
Quatro faixas horizontais empilhadas, cada uma com o nome da marcha e a insígnia correspondente à esquerda. Entram em cascata, de cima para baixo, com leve atraso entre elas.

**Condução**
Leia os quatro nomes em voz alta, na sequência, sem comentar entre eles. Os nomes fazem o trabalho. Comente só ao final: _"Repare na progressão do substantivo: recruta, combatente, guerreiro, soldado. É a mesma pessoa em quatro estágios."_

**Fonte** — 1.6; 5.2

---

### Tela 19 — Os símbolos

**Tela**

**Quatro insígnias. Nenhuma é troféu.**

_Toque em cada uma para ver o sentido bíblico._

**Visual**
Quatro cartões viráveis. Frente: a insígnia e seu nome duplo (bíblico + militar) — Cinto da Verdade / Cinto de Guarnição; Couraça da Justiça / Colete Balístico; Calçados do Evangelho da Paz / Coturno; Escudo da Fé / insígnia final. Verso: significado espiritual e pedagógico, com referência em Ef 6, em texto curto.

**Condução**
Chame atenção para o nome duplo: _"Cada peça tem o nome que a Escritura dá e o nome que a caserna reconhece. Essa é a ponte pedagógica inteira do projeto em duas palavras."_

**Fonte** — 1.6 (tabela)

---

### Tela 20 — Os ritos

**Tela**

**Sobriedade é regra, não estilo.**

O que a transição **é**: leitura bíblica breve, oração, entrega da insígnia, testemunho, registro pastoral simples.

O que a transição **não é**: premiação, ranking, comparação, espetáculo, condicionamento de bênçãos.

Nenhum discípulo deve ser constrangido ou exposto.

**Visual**
Duas colunas em contraste — "é" / "não é". A coluna da direita em cor mais apagada, quase riscada. Nenhuma animação. Deliberadamente austera.

**Condução**
Tela de salvaguarda. Ela existe para responder, **antes que seja feita**, a preocupação pastoral mais previsível: que a simbologia militar produza vaidade. _"O Guia condena triunfalismo em cinco seções diferentes. Isso não é ressalva de rodapé — é critério de condução."_

**Fonte** — 5.1; 1.6

---

### Tela 21 — Os certificados

**Tela**

**Memória pastoral, não comprovação de mérito.**

> "Certificamos que [nome] concluiu, pela graça de Cristo, a [marcha correspondente] do Discipulando a Caserna, como testemunho da obra que Deus começou e haverá de completar (Fp 1.6)."

Onde o contexto institucional não permitir a entrega física, **o registro pastoral simples cumpre a mesma função.**

**Visual**
Mockup do certificado da Primeira Marcha, em proporção real, com o escudo, a insígnia do cinto e o texto-base. Ao lado, miniaturas das outras três marchas. Se a arte definitiva ainda não existir, apresentar como **proposta**, rotulada como tal.

**Condução**
Segunda aparição da frase-âncora: ao mostrar o certificado, diga _"de quebrantados, valentes"_ — desta vez apontando o nome em branco no documento. É o momento mais emotivo da apresentação. Não o alongue.

**Fonte** — 5.3

---

# ATO VII — A IDENTIDADE VISÍVEL

> Função: tornar tangível. **Ato de maior risco pastoral** — a ordem das telas aqui é deliberada e não deve ser alterada. Quatro telas.

---

### Tela 22 — A logomarca é um catecismo

**Tela**

**O escudo ensina o que o programa confessa.**

Um escudo com as insígnias cravadas — o cinto, a couraça, os calçados e, no quarto campo, o próprio escudo do Projeto.
Encimado pelo **capacete**. Atravessado pela **espada**.

O discípulo não coleciona medalhas. **Ele é revestido por Cristo.**

**Visual**
A logomarca em tela cheia (`assets/img/logo-pdac/LOGO_DaC_Master_Flat_2D_Color.png`). É a única tela em que a marca aparece grande. Rotular como **logomarca oficial — apreciação pastoral pendente**. Inventário: `assets/img/logo-pdac/LEIA-ME.md`.

**Condução**
**Esta tela precisa vir antes de qualquer aplicação.** Ela estabelece que a identidade visual é doutrina antes de ser design. Só depois disso é legítimo mostrar a marca aplicada em objetos.

**Fonte** — 1.6 (parágrafo final)

---

### Tela 23 — A Caderneta de Campanha

**Tela**

**Não é brinde. É proposta de instrumento do método.**

Toda lição termina em **Ordem do Dia**: ação prática, simples e verificável, cumprida ao longo da semana — exigência do Guia (3.4).

A Caderneta seria onde ela é registrada — versículo memorizado, reconciliação buscada, serviço prestado, oração mantida.

Ela nasce da pedagogia, não da loja. **Ainda não é entregável aprovado.**

**Visual**
Mockup da caderneta aberta, rotulado **PROPOSTA**, mostrando o miolo: página de Ordem do Dia com campos (lição, data, ação, registro). O miolo importa mais que a capa — mostre-o maior.

**Condução**
**Tela decisiva do ato.** Reenquadra o que vem depois. Diga: _"O Guia exige a Ordem do Dia. A caderneta é só uma forma que estou propondo para registrá-la — não um produto já decidido."_

**Fonte** — 3.4 (Ordem do Dia); 3.3 · _nome “Caderneta de Campanha” = proposta desta apresentação_

---

### Tela 24 — Identidade de tropa

**Tela**

**Senso de corpo tem forma visível — em estudo.**

Camisa · Caneca · Adesivo · Insígnia bordada · Bolsa de campanha

Não são produtos de vitrine. São sinais de pertencimento a uma tropa — o mesmo princípio que o Guia assume quando fala de senso de corpo, camaradagem e vida comunitária.

**Salvaguardas propostas:**
Uso vinculado a discípulos, instrutores e apoiadores do projeto · Nenhuma peça condiciona participação, avanço ou bênção · Destinação de eventual receita definida e prestada em conta à liderança · Aprovação prévia de toda aplicação da marca

**Visual**
Galeria em grade, imagens em tom sóbrio, selo **ESTUDO — NÃO APROVADO** em cada peça (ou faixa única sobre a grade). **As salvaguardas aparecem na mesma tela**, peso tipográfico equivalente ao dos produtos — não em rodapé nem em tela seguinte. Ordem do ato permanece: 22 (doutrina) → 23 (caderneta) → **24 (tropa)** → 25 (governança).

**Condução**
Não venda. Apresente e cale. Se houver hesitação: _"nada disso avança sem a aprovação do senhor, e a destinação de receita é decisão da liderança, não minha."_ Registre a reação no checklist da Tela 29 (item de política de marca).

**Fonte** — 1.1 (senso de corpo); 5.1; menção a camiseta em ritos · _demais itens = proposta_

---

### Tela 25 — Governança da marca

**Tela**

**A forma pode variar. O conteúdo teológico, não.**

O Guia já estabelece: o recurso gráfico de cada símbolo pode mudar conforme o que estiver disponível — impresso, cartão, insígnia, projeção.
**O significado espiritual e a ordem dos símbolos não devem ser alterados.**

Pendência registrada: **Caderno de Identidade Visual do Projeto**, previsto no item 5.3 e ainda não produzido.

**Visual**
Bloco de governança sóbrio, com a pendência destacada em faixa. Sem imagens.

**Condução**
_"O Guia previu esse caderno. Ele ainda não existe. Proponho que seja o próximo entregável — sob a supervisão do senhor."_

**Fonte** — 1.6; 5.3

---

# ATO VIII — O HORIZONTE

> Função: mostrar que não é um fim, e que a expansão tem critério. Três telas.

---

### Tela 26 — Os Apêndices Pastorais

**Tela**

**Duas frentes de socorro, já previstas e ainda não desenvolvidas.**

**A Marcha da Liberdade** — associada ao Capacete da Salvação
**Vigília na Tempestade** — associada à Espada do Espírito

Mantidas em documento companheiro próprio, fora do corpo do Guia, por serem intervenção pastoral focada.

**Limites de escopo, explícitos:** não substituem acompanhamento profissional, jurídico ou clínico; não fazem promessas sobre processos institucionais; conduzidos em comunhão com a igreja e, havendo, com a capelania.

**Visual**
Dois cartões, ambos marcados com selo "em planejamento". Os limites de escopo abaixo, em bloco de igual peso visual.

**Condução**
_"Não vou apresentar isso como pronto, porque não está."_

**Fonte** — 8.1; 1.6

---

### Tela 27 — Formação de instrutores

**Tela**

**Multiplicação fiel, não reprodução vazia.**

O Guia forma o discípulo. Falta formar quem forma.

Critérios já estabelecidos para qualquer expansão:
**Fidelidade doutrinária** — não relativizar o que o Guia estabeleceu
**Coerência pedagógica** — não pular etapas por pressa
**Integridade pastoral** — não sacrificar cuidado de almas por alcance

**Visual**
Diagrama simples de multiplicação: um instrutor → uma turma → discípulos que se tornam instrutores. Os três critérios circundam o diagrama como condições, não como etapas.

**Condução**
_"O item 6.4.2 já prevê a multiplicação. O que não existe ainda é o percurso formal de formação do instrutor. É uma pendência que trago com transparência."_

**Fonte** — 6.4.2; 8.1; 2Tm 2.2

---

### Tela 28 — Do presídio à igreja

**Tela**

**O ambiente muda. O discipulado, não.**

O homem que concluiu o Módulo 1 no presídio **não recomeça do zero na igreja.**
Ele é reconhecido como quem já passou pela fase do chamado, e avança para o treinamento.

Uma única linguagem. Uma única progressão. Válida em presídio, quartel e igreja local.

Para isso, é preciso fluxo pastoral entre quem discipula dentro e quem recebe fora.

**Visual**
Três ambientes ligados por uma linha contínua — presídio, quartel, igreja — com a barra dos quatro símbolos atravessando todos, sem interrupção. A continuidade da linha é o argumento.

**Condução**
Última tela de conteúdo antes da decisão. _"Isso exige que a igreja seja avisada do estágio do discípulo. É uma decisão de fluxo — e depende do senhor."_

**Fonte** — 1.4.3

---

# ATO IX — A DECISÃO

> Função: converter apresentação em ato. Duas telas. O Prefácio entra aqui como **convite**, não como fato.

---

### Tela 29 — O que trago para decisão

**Tela**

**Pastor, o Guia está em v1.0-RC. Nenhum conteúdo é final antes da sua homologação.**

☐ **Apreciação doutrinária e pastoral** do Módulo 1 (doze lições, edições Aluno e Instrutor)
☐ Homologar o Guia Mestre v1.0-RC → v1.0
☐ **O prefácio do Guia Mestre** — página reservada; ver convite abaixo
☐ Liberação para produção dos Módulos 2 a 4
☐ Autorizar a turma-piloto _(pergunta de implantação: local, período, número)_
☐ Nomear o instrutor ou instrutores responsáveis _(pergunta de implantação)_
☐ Aprovar o estudo de identidade visual e a logomarca apresentados
☐ Autorizar a produção do **Caderno de Identidade Visual** (pendência do item 5.3)
☐ Autorizar o desenvolvimento dos **Apêndices Pastorais**
☐ Definir a política de aplicação da marca e destinação de receita _(Tela 24 — estudo de identidade de tropa)_
☐ Definir o fluxo de comunicação com a igreja local (item 1.4.3)

---

**CONVITE — O prefácio**

Há uma página do Guia Mestre que continua em branco por decisão minha: o prefácio.

Não é uma formalidade de abertura, e não peço que seja escrito por cortesia. O Guia descreve um método, mas quem pode dizer se esse método é fiel ao Cristo das Escrituras — e se serve de fato aos homens para quem foi escrito — não é o autor. É o pastor que acompanhou a obra desde a origem, que conhece os nomes por trás dos exemplos e que responderá diante de Deus pelo que for ensinado em seu nome.

Por isso, aquela página não foi preenchida com texto provisório à espera de aprovação. Está reservada, e permanecerá reservada pelo tempo que for necessário. **Não há prazo.** Se o senhor entender que o material ainda não está em condições de receber esse prefácio, essa também será uma resposta — e uma resposta que eu acolherei.

Quando quiser escrevê-lo, basta enviar o texto. Ele entra no Guia exatamente como o senhor o redigir, sem edição de conteúdo, acima da minha nota de autor.

---

**Visual**
Checklist funcional, marcável. Bloco CONVITE com peso tipográfico máximo da tela (borda latão / filete, respiro generoso) — o mesmo papel que a Seção 15 do prospecto dá ao prefácio. Botão "gerar resumo das decisões" (texto copiável com data, itens e observações). Botão "imprimir" ao lado.

**Condução**
**Não passe rápido.** Leia os itens em voz alta. No bloco do prefácio, leia o convite completo — é storytelling e ato ao mesmo tempo. Se ele decidir algo na hora, marque na frente dele. Se pedir tempo, ofereça o resumo impresso.

**Fonte** — nota de encerramento do Guia; `conteudo/secoes-12-15-a-prova-e-o-pedido.md` (Seção 15); 5.3; 8.1; 1.4.3

---

### Tela 30 — Fechamento

**Tela**

**De quebrantados, valentes.**

> "Estou plenamente certo de que aquele que começou boa obra em vocês há de completá-la até o Dia de Cristo Jesus."
> — Fp 1.6

Esse versículo está no certificado, e está aqui pelo mesmo motivo. O que foi construído até agora não é obra minha, e o que vier também não será. Fui apenas quem organizou, em papel, aquilo que já se via acontecer numa caverna.

Fico à disposição para o que o senhor determinar.

```
Obr. Flávio Alves da Costa
Projeto Caserna de Adulão · Fortaleza-CE
```

_Projeto Caserna de Adulão · CNPJ 63.724.286/0001-78_
_Guia Mestre v1.0-RC — documento de validação · circulação restrita · Fortaleza, 2026_

**Visual**
Retorno ao off-white vazio da Tela 01 — **fechamento visual do ciclo.** A barra de armadura no topo, agora completa, com as quatro peças acesas. É a única tela em que a barra está inteira.

**Condução**
Terceira e última aparição da frase-âncora (**Dedicatória**, não Prefácio). Depois da citação de Fp 1.6, silêncio. Não feche com frase atribuída ao Pastor.

**Fonte** — Dedicatória; 5.3; Seção 15 do prospecto; ficha institucional

---

# Notas de produção

## Elementos transversais

**Barra de armadura (topo, fixa)** — Aparece a partir da Tela 09. Quatro ícones vazados que se preenchem conforme o avanço: cinto na Tela 10, couraça na 18, coturno na 22, escudo na 30. Clicável para navegação por ato.

**Menu lateral retrátil** — Os nove atos, não as trinta telas.

**Modo leitura** — Alternador que revela, abaixo de cada tela, as notas de condução em texto corrido.

**Rodapé de versão** — Discreto, em todas as telas: `v1.0-RC · documento de validação`.

## Restrições técnicas inegociáveis

1. **Arquivo único**, sem CDN, sem fontes remotas, sem dependência de rede.
2. **Fontes embutidas em base64** ou pilha de fontes de sistema com fallback testado.
3. **Peso alvo abaixo de 3 MB.** SVG inline sempre que possível.
4. **Legível em celular.** Corpo mínimo de 17px; Tela 16 com tratamento próprio em tela estreita.
5. **`@media print` completo** — dossiê paginado; checklist da Tela 29 imprimível e assinável.
6. **Sem `localStorage`.** Estado do checklist em memória; exportação por cópia de texto.

## Paleta e tipografia propostas

| Uso                   | Cor                    |
| --------------------- | ---------------------- |
| Fundo base            | `#F4F4F1` off-white    |
| Texto principal       | `#2B2B2B` chumbo       |
| Estrutura e filetes   | `#4A4A4A`              |
| Insígnias e destaques | `#8C6A46` latão        |
| Acento institucional  | `#1A2A44` azul-caserna |

**Evitar:** preto absoluto, vermelho, estética tática-agressiva, caveira/armamento, gradientes vibrantes.

**Tipografia:** serifa de leitura para textos bíblicos e prosa; sans condensada em caixa alta apenas para rótulos militares e nomes de marcha. Duas famílias, no máximo. Preferir fontes já locais do prospecto (Montserrat + Source Serif 4), embutidas no HTML único.

## Assets disponíveis no repositório

- `assets/img/logo-pdac/` — logomarca oficial do Discipulando a Caserna (ver `LEIA-ME.md`)
- `assets/img/marca-escudo.svg`, `assets/img/brasao.svg` — estudos legados (não ativos na UI)
- Sem fotografias reais; sem arte de capa do Guia — tipografia + marca oficial.

## O que ainda precisa de confirmação humana antes de codificar

1. ~~Exibir ou omitir a Tela 24~~ — **decidido: exibir** (estudo proposto + salvaguardas)
2. Se o HTML único será artefato **irmão** do prospecto multiarquivo — **decidido: irmão** em `prototipos/homologacao-pastoral-v1/index.html` (gerado por `npm run generate:discipulando:apresentacao`; caminho legado: `apresentacao/homologacao-pastoral.html`)

---

_Roteiro v1.1 — ajustado após auditoria contra Guia Mestre v1.0-RC (revisão de 10/06/2026) e `conteudo/`. Prefácio tratado como convite. Afirmações em tela rastreáveis ao campo Fonte._
