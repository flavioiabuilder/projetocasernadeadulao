...
## Project scope (Discipulando a Caserna / Caserna de Adulão)

Prefer **detect-only / audit** on `programas/discipulando-a-caserna/conteudo/**`. Do **not** rewrite literal blockquotes (`>`), Scripture (NAA), or invent institutional claims while “cleaning” prose. Pastoral voice and `.cursor/rules/discipulando-caserna.mdc` override generic rewrite advice. Safe rewrite targets: UI chrome, agent-authored drafts outside canonical `conteudo/`.
