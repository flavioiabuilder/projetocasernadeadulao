...
| 5    | Prototipagem com IA      | Protótipo aprovado     | [`fases/fase-5-prototipagem.md`](fases/fase-5-prototipagem.md)         | Canônico: `prototipos/prospecto-fase-5-v1/` (F5-08/F5-10). Docs em [`docs/metodo/fase-5/`](../programas/discipulando-a-caserna/docs/metodo/fase-5/)                                                                            | **APROVADA (canônico técnico)** |
| 6    | Implementação            | Código em repositório  | [`fases/fase-6-implementacao.md`](fases/fase-6-implementacao.md)       | **LIBERADA** após F5-12 (`autorizacaoFase6: true`) — **não iniciada**. Sem criar `prospecto/`; sem publicar produção. **Publicação:** prévia F5 SUSPENSA (PUB-F5-01); DS interno; Pages em allowlist restrita + gate Qualidade | **LIBERADA (não iniciada)**     |
...
Fases 0–4 têm prompt próprio e validação semântica no gate. A Fase 5 tem prompt
[`prompts/prototipagem.md`](prompts/prototipagem.md) + crítica estruturada e um
check leve de estado do protótipo canônico no piloto; o julgamento visual
permanece humano. Fases 6–8 têm guia em [`fases/`](fases/) e checklist.
...
- A Fase 8 alimenta a Fase 1 seguinte: é o que faz o décimo projeto ser melhor
  que o primeiro.
- Voltar da 5 para a 2 é normal. Sequência: avaliação inicial (sem editar) +
  até três rodadas corretivas (estrutura → tipografia → cor/detalhe/movimento).
  Se ainda não convence, o problema é direção, não execução.
