...
| `.hero-special_bg-visual-wrapper`: 2 `<img>` empilhados (`.is-back` z-index auto, `.is-front` z-index 3), wrapper `scale(1.02)` + `overflow:clip`, camadas `scale(1.1)`, `object-fit:cover`. Sem `mask-image`, sem `mix-blend-mode` — a composição (front visível sobre back) é resolvida no canal alfa do próprio arquivo de imagem (confirmado no header VP8X do asset), não em CSS. Amostrado em 9 posições de **scroll** (0–1000px): `transform` idêntico em todas — **estático em relação ao scroll**. Mas **não estático em relação ao cursor** — ver linha de parallax por cursor abaixo (achado da P13, depois de uma checagem inicial incompleta que só testou scroll). | **P9 (continuação) — inspeção direta dos elementos + CSSOM** |
