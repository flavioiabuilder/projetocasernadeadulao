...
| `.hero-special_bg-visual-wrapper`: 2 `<img>` empilhados (`.is-back` z-index auto, `.is-front` z-index 3), wrapper `scale(1.02)` + `overflow:clip`, camadas `scale(1.1)`, `object-fit:cover`. Sem `mask-image`, sem `mix-blend-mode` — a composição (front visível sobre back) é resolvida no canal alfa do próprio arquivo de imagem (confirmado no header VP8X do asset), não em CSS. As duas `<img>` em si são estáticas no scroll (P9: 0 variação de `transform` própria em 9 posições, 0–1000px) — mas o **wrapper que as contém não é** (ver P14 abaixo) e nenhuma delas é estática em relação ao **cursor** (ver P13 abaixo). "Estático" na P9 original estava incompleto: só tinha testado scroll. | **P9 (continuação) — inspeção direta dos elementos + CSSOM** |
