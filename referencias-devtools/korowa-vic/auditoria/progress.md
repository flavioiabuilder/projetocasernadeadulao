...
- **Correção arquitetural (P15)**: a Friso travava a rolagem por 180vh com um "pin" (`.fr-pin-chapter`, `position:sticky`) durante o efeito de zoom/tingimento do hero — hipótese da sessão 2, nunca cruzada com a P9, que já tinha `pin: false` no `ScrollTrigger` real. Usuário reportou visualmente ("a imagem ficou travada... a referência rola ao mesmo tempo"). Confirmado por medição de posição (`getBoundingClientRect().top` muda 1:1 com `scrollY`, sem platô). Removido `.fr-pin-chapter`/`.fr-pin-chapter__sticky`/`data-fr-pin*` inteiramente; `bindPinProgress` virou `bindHeroScrollFx`, progresso = `scrollY / 510px` (token `--fr-scroll-hero-fx-range-px`), sem pin. Evidência: `raw/p15-hero-not-pinned-1440x900.json`. Corrigidas todas as menções a "pin de hero" em `README.md` (índice), `components.md`, `motion-system.md`, `NARRATIVE-MAP.md`, `DESIGN_AND_EFFECTS.md`.
- **Trigger global do GSAP identificado** (era `NÃO OBSERVADO` na P9 inicial): `vars = {start:'top+=50', end:'max', onUpdate, onToggle, toggleActions:'play'}`, sem `animation`/`pin` — ScrollTrigger só-callback, consistente com um driver de barra de progresso custom-coded. Confirma que a rail da Friso (`data-fr-progress`) tem análogo real, não é só hipótese. `docHeight` variando entre cargas (12339 vs 14679px) explicado: conteúdo dinâmico da página real, não erro de medição.
