...
This prospecto is a **static HTML/CSS/JS** pastoral presentation with an existing visual identity. Default to mode **3 (HTML / data / type)** unless the user explicitly asks for video scrub or Three.js.

Preserve incumbent brand, typography, and content from `conteudo/`. Prefer progressive enhancement + `prefers-reduced-motion` + readable document flow without JS. Do not impose Anthropic duotone aesthetics, custom cursors, or CDN frameworks. Do not invent institutional facts, endorsements, or results for "proof" beats.
